...
def test_put() -> str:
    request_header = "PUT /nop/put.html HTTP/1.1\r\nHost: xx\r\nContent-Length: 11\r\n\r\nPUT REQUEST\r\n\r\n"
    http_response = send_request(request_header)
    if http_response.status != 201:
        return "Bad status code: {}, expected: {}".format(
            str(http_response.status), "201"
        )
    if http_response.headers["Location"] != "/nop/put.html":
        return "Bad Location header: {}, expected: {}".format(
            http_response.headers["Location"], "/nop/put.html"
        )
    try:
        f = open("../../www/upload/put.html", "r")
    except:
